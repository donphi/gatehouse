...
def get_gate_home() -> str:
    """Get $GATE_HOME, defaulting to the auto-discovered package directory."""
    return str(_get_gate_home())
...
def cmd_new_rule(args):
    ...
    template_config = load_yaml(str(cd / "templates" / "rule_template.yaml"))
...
def cmd_init(args):
    ...
    schema_path = _schemas_dir() / f"{schema_name}.yaml"
...
def cmd_list_rules(args):
    ...
    if args.schema:
        ...
        schema_file = _schemas_dir() / f"{args.schema}.yaml"
...
def cmd_test_rule(args):
    ...
    rule_path = _rules_dir() / f"{rule_id}.yaml"
    ...
    import subprocess
    engine_module = "gatehouse.gate_engine"
...
""")
        ...
        test_schema_path = _schemas_dir() / "test-single-rule.yaml"
        import shutil
        shutil.copy(temp_schema.name, str(test_schema_path))

        result = subprocess.run(
            [sys.executable, "-m", engine_module, "--file", filepath, "--schema", temp_config.name],
...
def cmd_disable_rule(args):
    ...
    schema_data = load_yaml(str(schema_path))
    ...
    try:
        import yaml
        with open(str(schema_path), "w", encoding="utf-8") as f:
...
def cmd_status(args):
    ...
    rd = _rules_dir()
    sd = _schemas_dir()
    rules_ok = rd.is_dir() and any(f.endswith(".yaml") for f in os.listdir(rd))
    schemas_ok = sd.is_dir() and any(f.endswith(".yaml") for f in os.listdir(sd))
    print(f"  Rules:     {color('found', 'green') if rules_ok else color('NOT FOUND', 'red')} ({rd})")
    print(f"  Schemas:   {color('found', 'green') if schemas_ok else color('NOT FOUND', 'red')} ({sd})")
